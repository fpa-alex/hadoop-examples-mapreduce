...
public class ExistingSpeciesCount {
    public static void main(String[] args) throws Exception {
        Configuration conf = new Configuration();
        String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
        if (otherArgs.length < 2) {
            System.err.println("Usage: existingspeciescount <in> [<in>...] <out>");
            System.exit(2);
        }
        Job job = Job.getInstance(conf, "existingspeciescount");
        job.setJarByClass(ExistingSpeciesCount.class);
